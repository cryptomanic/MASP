...
\item \textbf{OIMASP:} A sequence of items $ I = \{I_{1}, I_{2}, I_{3}, ..........., I_{n}\} $ will be an \textbf{OIMASP} iff $ \forall j $  $ \in $ $ {1, 2, ...., n} $ the subset $ I' = \{I_{1}, I_{2}, ......., I_{j}\} $ must satisfy
\begin{enumerate}[i)]
...
\item $ \forall $ rows, if the specified item($ I $) is present in the row then perform swapping to bring that item to the first column \ref{Fig 3}.
\item Shuffle rows until there is no row left which contains item $ I $ and appears below row which do not have item $ I $ \ref{Fig 3}.
\end{enumerate}
